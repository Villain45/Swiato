...
const Cart = () => {
  ...
  const changeQuantities = (action, i) => {
    changeQuantity(action,i);
  };

  const total = () =>
    cartData.foodItems.reduce((total, i) => total + i.quantity * i.price, 0);
  ...
  const cantPlaceOrder = () => {
    if (Order)
      toast.error(
        "Order cannot be placed, Since recent order is not yet completed"
      );
    else if (cartData.foodItems.length === 0) toast.error("Cart is empty");
  };

  return (
    <Layout>
      <Helmet>
        <title>Swiato | Cart</title>
      </Helmet>
        ...
        <div className={css.container}>
                        ...
                        <td>
                          <div className={css.counter}>
                            <Image
                              ...
                              objectFit="contain"
                              className={css.img1}
                              onClick={() => changeQuantities("minus", i)}
                            />
                            ...
                            <Image
                              ...
                              objectFit="contain"
                              className={css.img1}
                              onClick={() => changeQuantities("plus", i)}
                            />
                          </div>
                        </td>
          ...
          <div className={css.cart}>
            ...
            <div className={css.cartDetails}>
              ...
              <div>
                <span>Total</span>
                <span style={{ fontFamily: "Arial", color: "var(--themeRed)" }}>
                  {" "}
                  &#x20B9;
                  <span
                    style={{ color: "var(--themeBlack)", paddingLeft: "3px" }}
                  >
                    {total()}
                  </span>
                </span>
            ...
            {!Order && cartData.foodItems.length > 0 ? (
              <div className={css.buttons}>
                ...
                <button className="btn" onClick={handleonOnlinePay}>
                  Pay Online
                </button>
              </div>
            ) : (
              <div className={css.buttons}>
                <button className="btn" onClick={cantPlaceOrder}>
                  Pay On Delivery
                </button>
                <button className="btn" onClick={cantPlaceOrder}>
                  Pay Online
